...
namespace HAMS.WebUI.Controllers
{
    ...
    public class AccountController : ApplicationBaseController
    {
        ...
        public FileContentResult Photo(string userId)
        {
            ...
            byte[] photo = user.ProfilePicture;

            if (photo == null)
            {
                Image img = Image.FromFile(@"D:\Універ\6 семестр\Програмування\HAMS\HAMS.WebUI\Content\Images\defaultAva.jpg");
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            var user = _unitOfWork.AspNetUserRepository.Get()
                                                    .Where(u => u.Email == model.Email)
                                                    .Select(u => new { u.Id, u.IsActive })
                                                    .FirstOrDefault();  
                    
            SignInStatus result;
            if (Helper.GetUserRole(user.Id) == "Candidate" || !user.IsActive )
            {
            ...
            switch (result)
            {
                case SignInStatus.Success: 
                    ...
                    string roleName = Helper.GetUserRole(user.Id);
        ...
        public ActionResult Register()
        {
            ...
            ViewBag.SpecializationList = _unitOfWork.GroupRepository.Get()
                                                                .Select(g => g.Specialization)
                                                                .Distinct()
                                                                .ToList();
            ViewBag.DepartmentList = _unitOfWork.GroupRepository.Get()
                                                            .Select(g => g.Department)
                                                            .Distinct()
                                                            .ToList();
            ViewBag.EnterYearList = _unitOfWork.GroupRepository.Get()
                                                            .Select(g => g.EnterYear)
                                                            .Distinct()
                                                            .OrderBy(g => g)
                                                            .ToList();
            return View();
